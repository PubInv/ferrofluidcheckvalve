...
two separated chambers, called $A$ and $Z$. The $A$ chamber is 3 times as high as the $Z$
chamber. The volume of the bolus is chosen so that its area between the glass sheets
is exactly 4 squares of length 1--3 in the chamber $A$ and 1 in the chamber $Z$.
...
\begin{align}
  ...
  \frac{\partial }{\partial x_Z}
 \Psi  \int_{0}^{ -4 + 3x_A} B(x) dx)
  && \text{by substitution} \\
  F_M =  & (3  \Psi B(x_A)  +
  - \Psi  B( -4 + 3x_A) ) && \text{by the Fundamental Theorem of Calculus} \\
  F_M =  &  \Psi ( 3 B(x_A)  +
   -B(-4 + 3x_A) ) && \text{by substitution} \\
  F_M =  & \Psi (\frac{3}{1+x_A^4}  +
    \frac{-1}{1+(-4 + 3x_A)^4}  ) && \text{by substitution}
  \label{eq:magforce}
\end{align}

Now we have an expression for $F_M$ as a function of $x_A$.
By maximizing $F_M$, we can find the maximum force that the bolus can withstand.
This force on the bolus due to magnetism is maximized at $x \approx -0.088917$
when $F_m = 2.9968 \Psi$. We can predict the bolus will collapse when the
...
where $x_A = \frac{4}{3}$. At this point, the force is $\frac{-94}{337}  \Psi \approx -0.278932  \Psi$.
To reach this point, the pressure force induced by the pressure from the $Z$
region is $Z P_Z = P_Z $ Newtons, or $P_Z = -0.278932  \Psi$.
Since $|-0.278932| < 1$, we have constructed a assymetric valve.

Air can pass from left to right at a pressure of about one third of the
pressure that will cause the valve to collapse if applied from the right.
Compared to a valve made out of solid steel or plastic, this would be considered
a very poor-performing valve. Nonetheless, it is unquestionable a
