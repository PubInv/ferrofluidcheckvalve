...
\begin{align}
  U_A = &\int_x^\infty B(q) f_A(q) dq \\
  U_Z = & \int_y^\infty B(q) f_Z(q) dq \\
\end{align}
This can be integrated by parts:
\begin{align}
  U_A = & B(\infty) F_A(\infty) - B(x) F_A(x) - \int_x^\infty B'(q) F_A(q) dq \\
  U_Z = & B(\infty) F_Z(\infty) - B(x) F_Z(x) - \int_x^\infty B'(q) F_Z(q) dq \\
\end{align}
It is clear that $B(\infty) F_A(\infty) = 0$ for any geometry, because even a
sphere feels decrease magnetic force as great distance become zero in the limit.

So:
\begin{align}
  U_A = & - B(x) F_A(x) - \int_x^\infty B'(q) F_A(q) dq \\
  U_Z = & - B(x) F_Z(x) - \int_x^\infty B'(q) F_Z(q) dq \\
\end{align}

If we differentiate this generally we have:

\begin{align}
  U_A = & - (B'(x) F_A(x) + B(x) f_A(x ))  - \int_x^\infty B'(q) F_A(q) dq \\
\end{align}
