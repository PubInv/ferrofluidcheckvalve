...
\section{To Do}
\begin{itemize}
\item Prove the manetostatic blob principle
\item Perform experiment of seeing where bubbles come out in a blob on the bottom of a magnet injected
  with air (hypo: will come out on bottom)
\item See if we can compute actaul force by placing a tube on a magnet and producing collapse force.
\item Note the position of the bolus could in fact be used to map the field strength if that had value.
\item Test the two new valves.
\item Develop theory with generalized appoximate B field.
  \end{itemize}
...
This math does not seem to line up. Right at the moment on this flight,
I don't have the ability to make it work, and can't use Wolfram Alpha
to check it. So I think we need to understand what I'm doing wrong here.

\subsection{Valve Pressure Performance Depends on Asymmetry of Valve}
