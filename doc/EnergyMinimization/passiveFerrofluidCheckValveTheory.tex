...
\subsection{Warmp: A Tube in a Bowl}

Let us consider the use of a simple bowl of water with a tube placed
vertically in it as a one-way valve.
Let the tube be inserted to a depth $D$ into the water.
Blowing into the tube, we willl have to overcome a restiance of $D$
centimeters of $H_2O$ pressure. When we exceed that, the valve will
``crack'' open, and the air will pass through, bubble out of the water,
and into the surrounding chamber.

Now suppose that we blow air into the surrounding chamber, which is
cut off from the tube. Water will start to rise up the tube.
This will resist a pressure equal to the height of the tube.
By making the tube very long, we have made a pretty good one-way valve.
The cracking pressure is relatively low, and the ``collapse'' pressure
is relatively high.  Nontheless, this is a simple one-way valve.

This valve has two major disadvantage:
\begin{itemize}
\item It depends on the force of gravity and the vertical orientation
  of the tube.
\item It has to be relatively large. To with stand a collapse pressure
  of 100 cm $H_2O$, it has to be one meter in the vertical dimenstion,
  which is quite large for a modest pressure.
\end{itemize}

We will see that by using a ferrofluid in a magnetic field, we can
build a valve withot these disadvantages.
...
(Note: This section assumes it is the slope of the field. I am now not sure of this.)

\subsection{Measuring $\Psi$}

I performed an experiment in a very thin carbon fiber tube. This tube appears
to have a 2mm inner diamter and 3mm outer diameter.

A ferrfofluid plug in this tube placed directly above (and centered) on a
standard 1/2'' x 1'' neobdynium magnet with stuff 280 cm H2O gauge pressure
in suction and 288 cm H2O gauge pressure in compressure. We will estimate
the value of $\Psi$ to be based 284 cm H2O:
\begin{align}
  \Psi =& 284 \text{cm H2O} / 2\text{mm}^2 \pi \\
  \Psi =& 27851 Pa / \pi 3.14159 10^{-6} \text{m}^2\\
  \Psi = 0.0875 \text{N}
  \end{align}


Note that this must be considered ideal. When we used a much larger tube
with a 1/4'' inner diameter, we could only resist 28 to 39 cm H2O.
Sadly more work is required to understand this.

\section{ An Extermely Simple Geometry }
\label{sec:simplegeometry}
...
Because the bolus is contiguous and we have contribed to give it one degree of freedom,
we can use $x_A$ to unambigously represent its postion.  The position of the edge
in the $Z$ chamber is a simple function of $x_A$, which we will denote $Z(x_A)$, which
is a signed negative value.
The $Z(x_A)$ border faces the $P_Z$ pressure. We will treat $P_A$ and $P_Z$ as unsigned values.
...
so that we can
describe the configuration of the bolus only with the positions $x_A$ and $Z(x_A)$.
...
can reach.
\begin{align}
  U_Z = & -\int_{Z(x_A)}^0 \int_{-\infty}^{\infty}  \Psi Z(x) dy dx \\
  U_Z = & -\Psi\int_{Z(x_A)}^0 Z B(x)  dx  \\
  U_Z = & - \Psi Z \int_{Z(x_A)}^0 B(x)  dx \\
...
is exactly the same (that is $X_A = 1 \rightarrow Z(x_A) = -1 \wedge
B(1) = B(-1)$.
...
air pressure from the left as caused $Z(x_A) = 0$, the bubble will move
...
To compute the cracking force, we neeed to compute the force at $Z(x_A) = 0$,
...
A weakness of the theory in this section is that the
volume of the bolus was chosen somewhat arbitrarily to
make the math and geometry easy to understand.
It provides no insight into if the valve would be more performant
with more or less ferrofluid.
...
\subsection{An Informal Explanation of the Theory}

It is valuable to have an informal explanation of why
the valve works.
There are two fundamental insights:
\begin{itemize}
\item a non-magnetic fluid that moves into a chamber with
  decreasing magnetic gradient will ``bubble out'' by following
  the gradient away from the magnet
\item if the geometry of the inlet is such that as
  the ferrofluid is pushed toward the stronger part of the field
  the fluid displaced into the outlet is moved into a strong field,
  it will partially offset the rise in potentially energy.
  In other words, the field in the outlet can act as a
  ``counterweight'' to the displacement of the inlet.
  Conversely, if the displaced fluid in the inlet is in
  a place of a low magnetic field, for example, by being
  far awy from the magnet, it will not balance the force
  needed to remove the outlet fluid from the field,
  thus allowing a high collapse pressure.
\end{itemize}

We might ask, is it possible to build such a valve
using simple water in a gravitational field.
I claim this is not possible under realistic circumstances
because the gravitation field does not vary in practice.

I believe it might be possible to build such a valve
out of water if it were place on the arm of centrifuge,
where centrifugal force varies linearly.
It is difficult to see the value of such a speciallized
situation, which is very far removed from lab-on-a-chip
or related fluid handling situations.

\section{ A More Performant 2D Radial Geometry  }

We reported on an early physical model of the PFCV in a
preprint\cite{stuckeyPreprint}
before we understood the theory presented here.
That model used a cylindrical magnet in a 2D plane using a
geometry similar to the geometry developed in the previous section.


After understanding the theory better and designing a fully
radial geometry and ensuring that our ferrofluid bolus
was smaller than the radius of our cylindrical magnet,
we improved our valve design, creating a radial 2D desgin,
a photograph of which is shown in Fig.~\ref{fig:2dRadial}.
A schematic of this geometry, which can be seen as
a modification of the linear system previously presented,
is depicted in Fig.~\ref{fig:ChannelChamber}.

\begin{figure}
\centerline{\includegraphics[width=4in]{figure/2DPFCV.png}}
\caption{2D Radial PFCV}
\label{fig:2dRadial}
\end{figure}

on the early results as defined here as
reported in Table~\ref{tab:best2D}.

This trial occured at an estimated bolus diameter of
0.355'', or an area of 0.396 square inches.
It was obseverd that if the bolus as as large
as the area of the cylindrical face of the magnet,
the cracking pressure rose drastically, destroying the performance.
This accord with our informal theory: if the edge of the bolus
in the outlet is {\em already} in a weak magnetic field because
it is too large, then it provides no counterbalance, causing the
cracking pressure to rise drastically.

This represents a performance ratio of greater than 7.
\begin{table}[]
  \label{tab:best2D}
  \begin{tabular}{l|l|l|l}
    \hline
...
\end{tabular}
\end{table}
\footnote{This data was collected with a manometer rated
for low, but which provided more accuracy than in other tables.}


\section{Theory of a 2D Radial Valve}

\begin{figure}
\centerline{\includegraphics[width=4in]{figure/ChannelChamber.png}}
\caption{2D Radial Valve Scheme}
\label{fig:ChannelChamber}
\end{figure}

The 2D radial design, when placed in the radial field created
at the end of a cylindrical magnet, can be seen as a mutation
of the linear valve presented in Sec.~\ref{sec:simplegeometry}.
It greatly increases the surface area in the outlet compared
to the surface area of the inlet, but allowing it to rotate
around nearly the full circumference of a circle.
This geometry forces us to use a polar coordinate system.
However, since it is almost complete radially symmetric,
we will never have to consider the angle in the polar
coordinate system.

The $B$ field of the magnet will be modelled the same
way, but we now think of it as a function of the distance
from the origin,$r$, rather than the $x$ coordinate:
\begin{align}
  B(r) =& \frac{1}{1+r^4}
  \label{eq:cylindricalmagnet}
\end{align}
Note: In Fig.~\ref{fig:ChannelChamber} there are two dashed circles.
The outer circle (heavily dashed) should be thought of as the outline
of the magnet. The dotted inner circle represents the circular edge
of the bolus, whose radius is denoted by $A$.
We used cylindrical magnets 1 inch long and 1/2 inch in
diameter,
We can take the units of $r$ in Eq.~\ref{eq:cylindricalmagnet}
to be in inches.
MagPyLib suggests the field of such a magnet is
actually a little steeper than Eq.~\ref{eq:cylindricalmagnet},
but we will continue to use this.

Our equation of the volume (or area) of the ferrofluid bolus
...
As before, $r_Z$ and $r_A$ are not independent variables,
and have one degree of freedom between the both of them.

We will typically choose $V$ so that the radius of
the bolus is about $1/2$ of radius of the magnet,
so approximately $1/8$ of an inch, leading
to a bolus area of $\frac{\pi}{64}$ square inches.

We neglect the width of the walls of the channel, which is inaccuracy which should be repaired.

Except for the change in geometry, our analysis is now very similar
to that of the 3:1 chamber analyzed earlier.
The channel potential energy is a two dimensional cartesian integration:
\begin{align}
  U_Z = & -\int_{0}^{r_Z} \int_{-k/2}^{k/2} \Psi B(r_Z) dy dr \\
  U_Z = & -\int_{0}^{r_Z} k \Psi B(r_Z) dr \\
  U_Z = & -k \Psi \int_{0}^{r_Z} B(r_Z) dr \\
  U_Z = & -k \Psi \int_{0}^{r_Z} \frac{1}{1+r^4} dr \\
  U_Z > & -k \Psi \frac{\pi}{2\sqrt{2}} && \text{by evaluation of the integral from $0$ to $\infty$}
\end{align}
But the chamber part is polar integration:
\begin{align}
  U_A = &  -\int_0^{r_A} \int_{-\pi}^{\pi} \Psi r B(r) d\theta dr \\
  U_A = &  -2 \pi \Psi \int_0^{r_A} r B(r) dr \\
  U_A = & - 2 \pi \Psi \int_{0}^{r_A} \frac{r}{1+r^4} dr \\
  U_A = & - \pi \Psi \arctan{r_A^2}
\end{align}
As before, the total potential energy is the sum of these:
\begin{align}
  U = & U_A + U_Z
\end{align}

As before, the force on the bolus is the change in potential
energy. We choose to analyzer this as the change in $r_A$.

By designing an geometry so that $k$ is small, we minimize
the change in potential energy $U_Z$ (and thereby the force) contributed by the
channel. This is especially as the bolus is driven out of the $A$ chamber
and into the $Z$ chamber, because a small change in radius in $A$,
represented by $r_A$, leads to a large change in $r_Z$, since the
fluid is proportional to the square of that radius, and $k$ has been
chosen to be small.  So we can reasonably present that:
\begin{align}
  \frac{\partial U_Z}{\partial r_A} \approx 0
  \label{eq:neglectchannel}
\end{align}

Note: This might NOT be necessary. Possibly I can leave it in please. The integral does
not produce elementary functions, but we are doing to immediately take the derivative of it anyway!


We can construct the force on the bolus the same way, but need to think of a positive
force as the force increasing $r_A$.

\begin{align}
  F_M = & - \frac{\partial U}{\partial r_A} \\
  F_M = &- (\frac{\partial U_A}{\partial r_A} +  \frac{\partial U_Z}{\partial r_A}) \\
  F_M = & \frac{\partial}{\partial r_A} \pi \Psi \arctan{r_A^2} + 0 && \text{ by Eq.~\ref{eq:neglectchannel}}  \\
  F_M = &  \frac{2 \pi \Psi r_A}{r_A^4 + 1} \\
  \label{eq:magforceradial}
\end{align}
This function is maximized at $ r_A = 1/3^(1/4 \approx 0.75984)$, and becomes nearly linear as $r_A$ approaches
$0$.

Howewver, when the pressure in chamber $A$ is high, the area that it can act on decreases as the {\em square}
of $r_A$, which is a critical point. In actual observation, as the presure in $A$ becomes higher
and higher, the bolus is driven further and further into $Z$ and does not collapse unto $r_A$ is near zero.
At any point before collapse, the forces will be in balance, so, using $F_P$ to represent pressure (note $F_P \leq 0$):
\begin{align}
  0 = & F_m + F_P \\
  0 = & \frac{2 \pi \Psi r_A}{r_A^4 + 1} + - \pi r_A^2 P_A \\
   \frac{2 \Psi }{r_A(r_A^4 + 1)} =&   P_A \\
\end{align}
This equation may be solved for $r_A$, but it depends, as we would expect, on $Psi /P_A$
From observation we believe the valve collapses wehn $r_A$ is quite small, approximately $k$.

I am unsure how to proceed. We don't care about $r_A$ when it is greater than about 0.5. If we neglect
the $Z$ regeion entirely, it may be hard to make this balance.
...
as depicted in Fig.~\ref{fig:ChannelChamber}.
...
\section{My Most Recent Thoughts}

I think I am starting to understand better.

I have ordered a constrained valve, on the theory that it will
decrease the cracking pressure.

I have ordered magnets.

\section{Some thoughts which are not necessarily valuable}
