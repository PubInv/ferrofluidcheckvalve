...
\begin{align}
  P(x) = -18.50670842 x^6 + -230.18413653 x^5 + -1174.49453555 + \\
  x^4 + -3154.6263118x^3 +
       -4734.06101237x^2 + -3819.38958143x + -1339.63590984 \\
       Q(x) = 18.50670842 x^6 + -230.18413653 x^5 + 1174.49453555 + \\
       x^4 + -3154.6263118x^3 +
       4734.06101237x^2 + -3819.38958143x + 1339.63590984 \\
B'(x) = &
\left\{
...
integral of $F$ such that $d/dy G(y,L) = F(y,L)$. Let $C(x)$ be the integral of $B'(x)$.
...
\left\{
    \begin{array}{lr}
      0 , & \text{if } x < -3 \\
      \text{anti-derivate of P(x))} , & \text{if } -3 < x  < -1/2 \\
      \alpha x^2/2 , & \text{if } -1/2 < x < 1/2 \\
     \text{anti-derivate of P(x))}   , & \text{if } 1/2 < x < 3 \\
      0 , & \text{if } x > 3
    \end{array}
\right\}
\end{align}
...
It is clear the collapse point will not occur until low-side edge is past creater than $1/2$. It is clear it will occur before the high-side edge reaches $1/2$. We therefore need only consider the bolus straddling
$1/2$. Let us assume that bolus $L < 1$, so that we need not consider the case that the high-side edge is
the left of $-1/2$.
Under these assumption, we have:
\begin{align}
  C(y+L/2) = & \text{Antiderivative of P(y+L/2)} \\
  C(y-L/2) = & \alpha (y-L/2)^2/2
\end{align}
Therefore the function $F(y,L)$ is a simple polynomial, which we can differentiate:
...
our experience. If you put a bolus in a thin straw subject
...
It is interesting to note that we obtain the same result by
...
so force is the change in potential energy divied by the distance:
\begin{align}
  F(y) = &  \frac{\Delta PE(y)}{\Delta y} \\
\end{align}
The limit of this as $\Delta y$ goes to zero is just the derivative of $PE(y)$:
\begin{align}
  F(y) = & PE'(y) \\
  F(y) = & \frac{d M}{1+(3y/2)^6}
\end{align}

In other words, Theorem~\ref{conj:sfc} gives us the same result
as integrating the entire space, but gives us a shortcut of allowing
us to integrate over only the edges, which in some cases can be
replace with simple algebra.
...
Note: I am now concerned that the concept of the ``force on the
blob'' may not even make make sense. I may instead have
to think in terms of force on one surface. The hydraulic prinicple
suggests this; pascal's principle is that pressure, not force is
conserved in the blob.

Considering only the surfaces and using Eq.~\ref{eqn:afromS}:
\begin{align}
  F^m_O(b) = & \pi b B(b) \\
  F^m_I(b) = & d B(a) \\
  F^m_I(b) = & d B(-\frac{\pi b^2 - 2 S}{2d})
\end{align}
\begin{align}
  F^m(b) = F^m_I(b) + F^m_O(b) \\
  F^m(b) = \frac{d}{1 + (-\frac{\pi b^2 - 2 S}{2d})^2} + \frac{\pi b}{1 + b^2}
\end{align}


Of course, $F^m(b)$ is a function of $S$ (the bolus area) and $d$, the channel
...
The cracking pressure will be the maximum force the magnet can exert
on the blob in the range $a < 0$, because when $a >= 0$, the
air or water being valved will ``bubble'' out of the valve.

The force exerted by pressure is the pressure times the area, or,
in our 2-dimentionsal model, the pressure times the length of an edge.
Since our edges may be curved and our ferrofluid does not support
shear forces, the force on a blob cause by a inlet or outlet pressure
is the pressure times the length across the blob (as opposed to the
arc length of the edge.)

We will denote the force on the blob due to magnetism by the superscript
$m$ and the force due to fluid pressure on the edges by $p$.

The outlet pressure force is thus:
\begin{align}
  F_{o}^{p} = P_o 2 b
\end{align}

Collapse occurs when the outlet pressure force exceeds the
maximum force $F_{o}^{m}(b)$. Both the magnetic force and
the pressure force depend on the position of the edge
given by $b$.


The collapse pressure will be the maximum force times the half-circumference
in the range $b >=0$.
By the time $b$ decreases to $b = 0$ (and probably before), the valve
will explosively collapse.

Note: I now question whether this can explain the behavior of the valve at all.

It is clear that we want the bolus to be somewhat larger then
