...
F(y) =
    ...
    The total force on the bolus is:
\begin{align}
  F(x) = & F_A(x-1) + F_B(x+1) \\
  F(x) = & d B(x-1) + -d B(x+1) \\
...
Utilizing our priniciple that only the position of the surfaces
matter,
we seek the force on the total bolus $F$.
The length of the surface in the B region is $\pi b$, since it
is a semicircle. The length of the surface in the A region is just $d$.
...
\begin{align}
  ...
  F_B(b) = & - \pi b B(b) \\
  F(b) = & d B(a) + - \pi b B(b) \\
  F(b) = & \frac{d}{1 + a^2} + -\frac{\pi b}{1+b^2}
\end{align}

which does not appear to be simplifiable.

But investigating further we can choose reasonable values and
at least check that shape of the function is reasonable.
Let us choose a size of the bolus that has a radius of
approximate 1, so let $S = \pi$. Let $d$ be thin compared to
this, so let $d = 1/10$. Then Wolfram alpha give us a plot:

\begin{figure}
\centerline{\includegraphics[width=6in]{figure/PlotOfIntermediateFunction.png}}
\caption{Plot of Force as a function of $b$}
\label{fig_plot}
\end{figure}
