...
\section{Theory of Operation (Best as of Apr. 24)}
...
B(r) = \frac{1}{1 + r^2}
\]
where $r$ is the distance from the orgin.
...
Just as we did before, we compute the integral over the bolus:

In need to decide if $y$ is measured as a displacement of $b$.
Probably best to make it a displacement of $b$ and replace $a$ in
the below
\begin{align}
  PE(0) = & \int_{a}^{0} d B(x)dx + \int_{0}^{b} \pi x B(x)dx\\
  PE(0) = & \int_{-\frac{\pi b^2 - 2 S}{2d}}^{0} d B(x)dx + \int_{0}^{b} \pi x B(x)dx\\
  PE(y) = & d\int_{-\frac{\pi b^2 - 2 S}{2d}+y}^{0} B(x)dx + \pi \int_{0}^{b+y} x B(x)dx\\
  PE(y) = & d\arctan{-\frac{\pi b^2 - 2 S}{2d}+y} + \pi \frac{1}{2} \log{(b+y)^2 +1}\\
  F(y) = & PE'(y) = \frac{d}{(-\frac{\pi b^2 - 2 S}{2d}+y)^2+1} + \pi\frac{b + y}{(b+y)^2 + 1}
\end{align}

Alternatively, considering only the interfaces,
\begin{align}
  F_B(b) = & \pi b B(b) \\
  F_A(b) = & d B(a) \\
...
When $d$ is small, we imagine that $F_A$ is much smaller than $F_B$.

To find the collapse pressure, we want to find the $b$ that maximizes of $F(b)$.
The cracking pressure is simpler, it occurs when $a = 0$, or
\begin{align}
  F_{\text{collapse}} = & \frac{d}{1 + (-\frac{\pi \frac{2S}{\pi} - 2 S}{2d})^2} + \frac{\pi \sqrt{\frac{2S}{\pi}}}{1 + \frac{2S}{\pi}} \\
  F_{\text{collapse}} = & \frac{d}{1 + 0} + \frac{\pi \sqrt{\frac{2S}{\pi}}}{1 + \frac{2S}{\pi}} \\
  F_{\text{collapse}} = & d + \frac{\pi \sqrt{\frac{2S}{\pi}}}{1 + \frac{2S}{\pi}}
\end{align}

If we assume $d$ is small, we may be able to ignore this.
Seeking the extrema of this, we find it occurs when $S = \pi /2$.
Note, that his is a function of filed $B(x)$ that we chose to model.
