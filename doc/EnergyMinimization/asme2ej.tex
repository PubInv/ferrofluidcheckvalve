...
Ferrofluids are colloidal suspensions of ferromagnetic particles in a non-polar, liquid carrier. 
In the work presented in this paper, a finite volume method based numerical study is performed to investigate the ferrofluid behavior in a passive valve using Ferrohydrodynamics. % Lisa ref 2
...
Note that $B(x)$ is an odd function; that is, $B(-x) = -B(x)$.
...
\begin{align}
  U_B = & - \pi \int_0^b r B(r) dr \\
  U_N = & -k\int_0^a B(r) dr\\
  U = U_B + U_N
\end{align}

This expresses our fundamental situation. Because force applied in the chamber moves the bolus
into the channel, and force applied in the channel moves the bolus into the chamber, the
potential energy changes in both. The potential energy always goes down as more fluid is added to one side (or
at least never goes down), and thereofore always goes up in the chamber from which it is removed.
The minimum potential energy is a point of balance. We seek a mathematical description of the
change in totoal potential energy as $b$ changes so that we can compute the force. This should
allow us to choose $S$ to be most advantageous. Eventually, this may allow us to design different
...
\begin{align}
  0 = & \frac{d F(b)}{db} \\
  0 = & \frac{d^2 U}{db^2}  \\
  0 = & \frac{d^2}{db^2} U_B +  \frac{d^2}{db^2} U_N \\
...
\begin{align}
  \frac{d}{db} \int_0^b B(r) r  = &
 \frac{d}{db} \int_0^b
  \left\{
    \begin{array}{lr}
      r \cdot c(r) dr , & \text{if } -1/2 < r \leq 1/2 \\
      r \cdot q(r) dr , & \text{if } 1/2 < r < 3 \\
      0  dr , & \text{if } r > 3
    \end{array}
    \right\} \\
  \frac{d}{db} \int_0^b B(r) r = &
...
\begin{align}
  0 = & - \frac{d}{db} \pi ( b \cdot q'(b) + q(b)) + - \frac{d}{db} (k q(A(b))) \\
