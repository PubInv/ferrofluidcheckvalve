...
\begin{align}
  F = & \int_{x-1}^{x+1} k B'(x) dx \\
  F = & k \int_{x-1}^{x+1} \frac{2x}{(1 + x^2)^2} \\
  F = & k \frac{4x}{4 + x^2}
\end{align}
...
The extrema of this force $F$ occur at
...
\begin{align}
  F_B = & - \int_{0}^{b} \pi \frac{r}{1 + r^2} \\
  F_B = & - \pi \int_{0}^{b} \frac{r}{1 + r^2} \\
  F_B = & - \frac{\pi\ln{(1 + b^2)}}{2}
\end{align}

We can now revisit our total force and eliminate $a$:

\begin{align}
  F = & F_A + F_B \\
  F = & d \arctan{a} + - \frac{\pi\ln{(1 + b^2)}}{2}
  \end{align}
