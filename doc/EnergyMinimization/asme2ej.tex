...
in this bolus at any point $y$ is in the positive $X$ direction
of the gradient of $B(y)$, $B'(y)$, is increasing, and
in the negative $X$ direction if it is decreasing (that is,
if $y > 0$. Qualitatively, the ferrofluid is pulled toward
the origin.
Let $k$ be a constant of proportionality that represents the
the absolute strength of the field and permeability\footnote{Is this the correct term?}
of the ferrofluid.
\[
F(y) = k B'(y)
\]
so as as $y$ is inside the bolus. Integrating the entire bolus
to find the force on it:
\begin{align}
  F(x) = & \int_{x-1}^{x+1} k B'(x) dx \\
  F(x) = & k \int_{x-1}^{x+1} \frac{2x}{(1 + x^2)^2} \\
  F(x) = & k \frac{4x}{4 + x^2}
\end{align}
...
$(-\frac{2}{\sqrt[4]{3}},\frac{2}{\sqrt[4]{3}})$, or $(-1.0746,1.0746)$, which
...
B(r) = \frac{1}{1 + r^4}
\]
where $r$ is the distance from the orgin. A given point
feels a force:
\[
B(r,\theta) = k \frac{1}{1 + r^4}
\]
in the direction $-\theta$.
...
We seek the force on the total bolus $F$, which is:
\[
F = F_A + F_B
\]
...
\subsection{Some Design Thoughts}

Assume that a blob in this section means an ideal 2-dimensional blob,
approximated by a blob constrained into a very thin plane.

\begin{conjecture}[Magnetostatic Blob Force]
  \label{conj:mbf}
  The magnetostaic force on a blob depends
  is the integral over the surface length in the magnetic gradient
  at every point on the surface.
\end{conjecture}

\begin{corollary}[Equigradient Blob Surface Principle]
  Conjecture~\ref{conj:mbf} implies that a Blob at rest acted on by
  no other forces has every surface at an equivalent magnetic gradient.
  (This corrollary depends on the reasonable assumption that the
  gradients are smooth and continuous.)
\end{corollary}
...
is equal to the gradient at a point multiplied by the surface
length.
In this scenario, the air pressure on the blob is a function
of the length of the surface, but the magnetostatic force is
also a function length, so the length cancels out and becomes
irrelevant.
The collapse pressure is therefore the maximum gradient
gradient at any point between the rest position and the origin
(this gradient is by construction equal on a semi-circular arc.)
The collapse pressure also depends on the gradient
...
making the gradient in the region B as shallow as possible
...
\begin{figure}
\centerline{\includegraphics[width=4in]{figure/IdealFieldProfile.png}}
\caption{Ideal Field Profile}
\label{fig:IdealFieldProfile}
\end{figure}
