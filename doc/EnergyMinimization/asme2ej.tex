...
Figure~\ref{fig:BoluSetup} shows our setup of the problem.
...
\caption{One Dimensional Bolus Math [ This Figure is now inaccurate. ]}
\label{fig_oneD}
\end{figure}

The $x$-axis in the channel at point $x$ is:
\begin{align}
  B(x) = & \int F_E(x-1) + F_W(x+1) \\
  B(x) = & d B(x-1) + -d B(x+1) \\
  B(x) = & M d (1/(1 + (3(x-1)/2)^6) + - 1/(1 + (3(x +1)/2)^6)) \\
\end{align}

Now imagine that bolus has a length of 2, and let the channel area be $d$, and let $x$ be
...
The $x$-axis force on a bolus in the channel generally is:
\begin{align}
  F(x) = & \int d B(x-1) + -d B(x+1) \\
  F(x) = & M d (1/(1 + (3(x-1)/2)^6) + - 1/(1 + (3(x +1)/2)^6)) \\
\end{align}

The force on the ferrfoluid
