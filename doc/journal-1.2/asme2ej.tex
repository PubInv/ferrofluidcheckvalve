...
Integral components to microfluidic systems include pumps and valves
to enable flow management. Current methods of enabling such control
include, but are not limited to, pneumatic and electrical means.
However, implementation of these mechanisms can prove costly and
increasingly difficult as the scale of the device decreases.  This
paper presents a novel passive ferrofluid check valve, without
utilizing mechanical or electrical input, implemented through only a
unique channel-and-chamber geometry, ferrofluid, and a stationary
magnetic field.  The proposed design, open to improvements through
various parameters, can be used for fluid handling and lab-on-a-chip
applications. The prototype and experimental setup are addressed along
with potential directions of research to refine the model to desired
...
fields to exert force on fluids[Torres-Diaz \& Rinaldi, Kone \&
  Khandekar]. This makes it possible to build pneumatic or hydraulic
devices, perhaps on very small scales such as a single chip, to
miniaturize fluid handling. This paper reports only on experiments
done with air. This could be used to make a “lab on a chip” (LOC) or
even to heat or cool different areas. A fundamental component of such
devices is the check or one-way valve. A perfect check-valve opens or
cracks with minimal pressure on the inlet side and resists maximal
pressure on the outlet side, allowing fluid to flow in only one
direction. Following [Hartshorne et al.] we call the maximum pressure
differential the valve can resist in the direction it is intended to
check (from outlet to inlet) the sustainable pressure. Two check
valves on either side of a chamber whose volume can vary creates a
pump. This paper presents an initial, preliminary design of a
functioning passive ferrofluid check valve (PFCV) that has no moving
parts except for the ferrofluid bolus itself. By passive, the authors
...
permanent magnet or an electromagnet. That is, the flow is determined
purely by the difference between the inlet port pressure and the
outlet port pressure. [Note: Harshorne, Backhouse and Lee use the
  “active” to simply mean a valve that is not a permanent seal. I
  think their language is confusing.]  To our knowledge, no passive
ferrofluid check valve has been reported before, despite being an
...
on micropump and lab-on-a-chip applications. Many of these papers use
a version of mechanical valve not based on passive
ferrofluid. Yamahata and Gijs and Yamahata and Chastellain et al.  use
a corrugated silicone micro valve[Yamahata and Gijs]. Menz et
al. describes an active T-Valve with a moving ferrofluid plug. Ando,
Ascio and Baglo describe a complete fluid pump with valves that use
active control of a ferrofluid bolus, which is possible because the
action of the plunger may be synchronized with the opening and closing
of the valves. Nonetheless a passive valve would be simpler and less
expensive, and would not require knowledge of the behavior of the
plunger. Hartshorne, Backhouse, and Lee similarly describe two kinds
of active valves, a well valve and Y valve. Paschalis and Chodosh
et. al describe a passive ferrofluid two-way valve with tunable
opening and closing pressure based on magnetic field strength. This
paper has not studied the closing pressure of the PFCV, but reports on
the opening (or cracking) pressure (for flow from inlet to outlet) and
sustainable pressure when the outlet pressure is higher than the inlet
side. [Hatch and Kamholz] describe a functional micropump in which the
moving ferrofluid bolus merges with a fixed ferrofluid valve and then
separates on each pumping cycle, but is not a one-way valve.

The PFCV is a simple asymmetric chamber centered in a magnetic field
which holds a ferrofluid bolus in place. It consists of a narrow
channel at the inlet opening onto a chamber on the outlet side. The
magnetic field is centered on the connection between the inlet channel
and the outlet chamber. The PFCV does not perform as well as a valve
made out of moving parts made of steel. That is, the sustainable
...
relatively high. However, it may operate reliably within a range of
known pressures, and thus be sufficient to build a
pump-on-a-chip. Furthermore, the PFCV is a preliminary design which
can probably be significantly improved. The authors found the
existence of the PFCV exciting enough to report on it immediately.

\section{Method}

To test the theory, the valve was constructed using Solidworks 2016.  The model consists of a 15 millimeter long, 2x2 millimeter channel, two female luers, a magnet holder ring  and two legs to provide room for the magnet.  Viewed from the top, one end opens up to a recovery chamber of circular profile 30 millimeters in diameter and the other an outlet chamber with a flat wall.  A magnet ring approximately half an inch in diameter was created centered on the channel-chamber junction, where the bolus is placed. On the inlet side the channel opens into a recovery chamber shaped to allow the ferrofluid to be passively drawn back into the channel by the magnetic field after a collapse of the bolus.  The model was printed on a Projet MJP 2500 (3D Systems, Rock Hill, SC), using Visijet M2G-CL and VisiJet M2 SUP as material and support respectively (3D Systems, Rock Hill, SC). Support material was removed by using an EasyClean system (3D Systems, Rock Hill, SC) and Dawn dish soap (Procter \& Gamble, Cincinnati, OH) to remove residuals.

%%%%%%%%%%%%%%%% begin figure %%%%%%%%%%%%%%%%%%%
%%% 3.34in is the maximum width you can have for a figure
\begin{figure}
\centerline{\includegraphics[width=3.34in]{figure/equipment.png}}
...
%%% 3.34in is the maximum width you can have for a figure
\begin{figure}
\centerline{\includegraphics[width=3.34in]{figure/diagram.jpeg}}
\caption{Example taken from a paper that was held from production because the image quality is poor.  ASME sets figures captions in 8pt, Helvetica Bold.}
\label{fig_example1.jpg}
\end{figure}
%%%%%%%%%%%%%%%% end figure %%%%%%%%%%%%%%%%%%%
...
to measure pressure. A 1/2” x 1” cylindrical neodymium magnet (Apex
Magnets, Petersburg, WV) was placed inside the magnet channel by means
...
syringe. Pressure applied from the outlet side of the model, in which
fluid flowed through the channel, will be referred to as the
sustainable pressure. Pressure applied from the inlet needed to
initiate flow will be referred to as the cracking pressure.
...
pushing the syringe plunger faster simply increases flow.) Then the
sustainable pressure was measured by increasing the pressure
...
Cracking Pressure (mmHg) &
Collapse Pressure (mmHg) &
Pressure DIfference (mmHg) &
Approximate Ratio: Cracking to Collapse Pressure \\
\hline
Single magnet &
8 (1.1) &
41 (5.5) &
...
configurations.  With two magnets, when recollecting the ferrofluid to
the center of the model (following a collapse of the bolus due to
exceeding the sustainable pressure), fluid further from the bolus
...
Thus, although the trial with two magnets demonstrated a larger
pressure difference due to magnet strength, the single magnet trial
granted a larger ratio via expanded field lines.
...
nothing but a channel, a chamber, and a bolus of ferrofluid in a
...
observed.  Obvious future research possibilities are: To improve the
performance by varying the geometry of the passive design or shape and
strength of the magnets.  Utilizing this design to make a micro-pump
similar to earlier micro-pumps but with this simpler check valve
design.  To provide an explanatory and predictive theory of operation,
for example based on magnetic field strength as per [Ando, Asco, and
  Baglio] .  Studying the ability of the valve to recover after a
collapse when high outlet pressure is reduced.
